...
public class GuiPrintOld extends GuiScreen
{
    ...
    public GuiPrintOld(IPrintHelper helper)
    {
        this(helper, null);
        this.util = new RenderUtility(this);
    }
    
    public GuiPrintOld(IPrintHelper helper, RenderUtility util)
    {
        super();
        this.helper = helper;
        this.clicked = false;
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException
    {
        ...
        {
            this.getPrint().posY -= GuiPrintOld.scrollSpeedInner;
        }
        ...
        {
            this.getPrint().posX -= GuiPrintOld.scrollSpeedInner;
        }
        ...
        {
            this.listOffset += GuiPrintOld.scrollSpeedNodeList;
        }
            ...
            if (this.inner.isCoordInside(mouseX, mouseY))
            {
                ...
                if (!this.clicked)
                {
                    ...
                    if (obj == null && this.mouseHoveringNodeES != null)
                    {
                        ...
                        {
                            if (this.mouseHoveringNodeES.canExpand())
                            {
                                this.mouseHoveringNodeES.expand();
                            }
                        }
                        else
                        {
                            if (this.mouseHoveringNodeES.canShrink())
                            {
                                this.mouseHoveringNodeES.shrink();
                            }
                        }
                    }
    ...
    public void drawInner(int mouseX, int mouseY, float partialTicks)
    {
        ...
        {
            this.util.drawNodeHoveringText(this.mouseHoveringNode, mouseX, mouseY);
        }
        
        if (this.mouseHoveringField != null)
        {
            this.util.drawNodeFieldHoveringText(this.mouseHoveringField, mouseX, mouseY);
        }
        ...
        if (this.clicked)
        {
            ...
            if (this.mouseClickedField != null)
            {
                ...
                if (this.mouseClickedField.isOutput())
                {
                    ...
                    RenderUtility.drawGradientLine(dotX + this.util.nodeFieldDotSize / 2, dotY + this.util.nodeFieldDotSize / 2, this.mouseXToPrintRounded(mouseX), this.mouseYToPrintRounded(mouseY), this.util.getLineWidth(this.mouseClickedField.dataType), this.mouseClickedField.dataType.getColor()[0], this.mouseClickedField.dataType.getColor()[1], this.mouseClickedField.dataType.getColor()[2], this.util.nodeFieldConnectionsAlpha, GuiPrintOld.nodeFieldDef, GuiPrintOld.nodeFieldDef, GuiPrintOld.nodeFieldDef, this.util.nodeFieldConnectionsAlpha);
                }
                else
                {
                    //input on mouse
    ...
    public void drawNodeInteractions(Node node, int x, int y)
    {
        if (!this.clicked)
        {
            ...
            if (node == this.mouseHoveringNodeES)
            {
                ...
                if (!this.expandShrink)
                {
                    this.drawHoverRect(x, y + (RenderUtility.getVerticalAmt(node) - 1) * this.util.nodeHeight, this.util.fieldWidth, this.util.nodeHeight);
                }
                //Shrink
                else
                {
                    this.drawHoverRect(x + this.util.fieldWidth, y + (RenderUtility.getVerticalAmt(node) - 1) * this.util.nodeHeight, this.util.fieldWidth, this.util.nodeHeight);
                }
            }
