...
{
    // The height of the header or an output/input NOT THE ENTIRE NODE WIDTH AS THEY
    ...
    // --- Mouse clicks on node or field -> temporarily (and additionally) stored
    // here ---
    //
    ...
    public void drawInner(int mouseX, int mouseY, float partialTicks)
    {
        ...
        if (obj instanceof Node)
        {
            ...
            GuiPrint.drawRect(this.print.posX + node.posX, this.print.posY + node.posY, nodeWidth,
                    nodeHeight * GuiPrint.getVerticalAmt(node), 1F, 1F, 1F, hoverAlpha);
        }
        else if (obj instanceof NodeField)
        {
            ...
            GuiPrint.drawRect(this.print.posX + field.node.posX + (field.isInput() ? 0 : nodeWidth / 2),
                    this.print.posY + field.node.posY + nodeHeight * (field.id + 1), nodeWidth / 2, nodeHeight, 1F, 1F,
                    1F, hoverAlpha);
        }
    ...
    public void drawNode(Node node, int x, int y)
    {
        ...
        GuiPrint.drawRect(x, y, nodeWidth, nodeHeight * GuiPrint.getVerticalAmt(node), nodeBackground, nodeBackground,
                nodeBackground);
    ...
    public void drawNodeHeader(Node node, int x, int y)
    {
        ...
        drawRect(x + 1, y + 1, nodeWidth - 2, nodeHeight - 2, node.getColor()[0], node.getColor()[1],
                node.getColor()[2]);
    ...
    public void drawNodeField(NodeField field, int x, int y)
    {
        ...
        // width - dot - border
        // the dot is in the middle of a quad of size height x height, at the left/right
        // of the field
        ...
        this.fontRenderer.drawString(name, nameX + 1, nameY + 1, 0xFFFFFFFF); // Draw the trimmed name, maybe add
                                                                              // shadow?
    }
            ...
            if (GuiPrint.isCoordInsideRect(mouseX, mouseY, x, y, w, h))
            {
                ...
                else
                {
                    ...
                    if (GuiPrint.isCoordInsideRect(mouseX, mouseY, x, y, this.guiToPrint(nodeWidth / 2), h))
                    {
                        ...
                        {
                            if (GuiPrint.isCoordInsideRect(mouseX, mouseY, x,
                                    this.guiToPrint(node.posY + nodeHeight * j), w, this.guiToPrint(nodeHeight)))
                            {
    ...
    {
        return i * this.zoom; // TODO today: see below as well, separate method to adjust for the position of
                              // the print
    }
    ...
    /**
     * (This is the almost-equivalent of the vanilla Gui class's drawRect method. The differences are direct byte rgba input without bit shifting, and width/height instead of max
     * x/y.) Draws a solid color rectangle with the specified coordinates and color.
     * 
